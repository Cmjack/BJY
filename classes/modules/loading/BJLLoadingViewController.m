...
- (void)makeConstraints {
    ...
    [self.exitButton mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        if (iPhoneXSeries()) {
            // ver
            make.top.equalTo(self.view.bjlmas_safeAreaLayoutGuide)/* .offset(- BJLViewSpaceS) */.priorityHigh();
        }
        else {
            // self.view.bjlmas_safeAreaLayoutGuide: iOS 11
            // self.mas_topLayoutGuide: earlier
            make.top.equalTo(self.view.bjlmas_safeAreaLayoutGuide ?: self.mas_topLayoutGuide).priorityHigh();
        }
        make.right.equalTo(self.view.bjlmas_safeAreaLayoutGuide ?: self.view).with.offset(- BJLViewSpaceM);
    }];
