...
- (void)makeConstraints {
    ...
    [self.exitButton mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        // iOS 11
        if (self.bjlmas_safeAreaLayoutGuide) {
            if (iPhoneXSeries()) {
                // ver
                make.top.equalTo(self.bjlmas_safeAreaLayoutGuide)/* .offset(- BJLViewSpaceS) */.priorityHigh();
            }
            else {
                make.top.equalTo(self.bjlmas_safeAreaLayoutGuide).priorityHigh();
            }
        }
        // earlier
        else {
        ...
        make.right.equalTo(self.bjlmas_safeAreaLayoutGuide ?: self).with.offset(- BJLViewSpaceM);
    }];
