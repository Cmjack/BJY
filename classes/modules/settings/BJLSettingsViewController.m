...
@property (nonatomic) UIView *micLabel, *cameraLabel, *beautifyLabel, *backgroundAudioLabel, *forbidChatLabel, *forbidSpeakLabel, *forbidAudioLabel;
@property (nonatomic) UISwitch *micSwitch, *cameraSwitch, *beautifySwitch, *backgroundAudioSwitch, *forbidChatSwitch, *forbidSpeakSwitch, *forbidAudioSwitch;
...
- (void)makeSubviewsAndConstraints {
    ...
    if (self.room.loginUser.isTeacherOrAssistant) {
        ...
        switch (self.room.roomInfo.roomType) {
            case BJLRoomType_1toN: {
                ...
                tuples = @[micControls, cameraControls, beautifyControls, backgroundAudioControls, forbidChatControls, forbidSpeakControls];
                break;
            }
            case BJLRoomType_smallClass: // NO break
            case BJLRoomType_smallClassV2: // NO break
            case BJLRoomType_doubleTeachersClass: {
                ...
                tuples = @[micControls, cameraControls, beautifyControls, backgroundAudioControls, forbidChatControls, forbidAudioControls];
                break;
            }
            default: {
                tuples = @[micControls, cameraControls, beautifyControls, backgroundAudioControls, forbidChatControls];
                break;
            }
        }
    ...
    [spaceView mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.right.equalTo(@[self.scrollView, self.view.bjlmas_safeAreaLayoutGuide ?: self.view]); // right
        make.top.bottom.equalTo(self.scrollView);
    }];
...
- (void)makeObservingAndActions {
    ...
    [self.backgroundAudioSwitch bjl_addHandler:^(UISwitch * _Nullable sender) {
        bjl_strongify(self);
        [sender bjl_disableForSeconds:BJLRobotDelayS];
        self.room.mediaVM.supportBackgroundAudio = sender.on;
    } forControlEvents:UIControlEventValueChanged];
    ...
    [self.forbidAudioSwitch bjl_addHandler:^(UISwitch * _Nullable sender) {
        bjl_strongify(self);
        [sender bjl_disableForSeconds:BJLRobotDelayS];
