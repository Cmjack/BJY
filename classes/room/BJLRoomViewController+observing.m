...
- (void)autoStartRecordingAudioAndVideo {
    ...
    BOOL audoOn = !self.room.recordingVM.forbidRecordingAudio;
    BOOL videoOn = (self.room.featureConfig.mediaLimit != BJLMediaLimit_audioOnly
                    && (self.room.loginUser.isTeacherOrAssistant || self.room.featureConfig.autoPublishVideoStudent));
    
    if (audoOn && videoOn) {
        [BJLAuthorization checkMicrophoneAccessAndRequest:YES callback:^(BOOL micGranted, UIAlertController * _Nullable alert) {
            if (!micGranted) {
                if (alert) {
            ...
            [BJLAuthorization checkCameraAccessAndRequest:YES callback:^(BOOL cameraGranted, UIAlertController * _Nullable alert) {
                if (!cameraGranted) {
                    if (alert) {
                        [self presentViewController:alert animated:YES completion:nil];
                        return;
                ...
                if (!micGranted && !cameraGranted) {
                    [self showProgressHUDWithText:@"麦克风、摄像头未授权"];
                    return;
                }
                
                BJLError *error = [self.room.recordingVM setRecordingAudio:micGranted recordingVideo:cameraGranted];
                if (error) {
                    [self showProgressHUDWithText:(error.localizedFailureReason ?: error.localizedDescription ?: @"麦克风、摄像头打开失败")];
                    return;
                }
                
                NSString *message = (micGranted && cameraGranted ? @"麦克风、摄像头已打开"
                                     : (micGranted ? @"麦克风已打开"
                                        : (cameraGranted ? @"摄像头已打开"
                                           : nil)));
                if (message) {
                    [self showProgressHUDWithText:message];
                }
        ...
        [BJLAuthorization checkMicrophoneAccessAndRequest:YES callback:^(BOOL granted, UIAlertController * _Nullable alert) {
            if (!granted) {
                if (alert) {
                    ...
                    [self showProgressHUDWithText:@"麦克风未授权"];
                }
                return;
            }
            
            BJLError *error = [self.room.recordingVM setRecordingAudio:YES recordingVideo:NO];
            [self showProgressHUDWithText:(error
                                           ? error.localizedFailureReason ?: error.localizedDescription ?: @"麦克风打开失败"
                                           : @"麦克风已打开")];
        }];
    }
        ...
        [BJLAuthorization checkCameraAccessAndRequest:YES callback:^(BOOL granted, UIAlertController * _Nullable alert) {
            if (!granted) {
                if (alert) {
                    ...
                    [self showProgressHUDWithText:@"摄像头未授权"];
                }
                return;
            }
            ...
            [self showProgressHUDWithText:(error
                                           ? error.localizedFailureReason ?: error.localizedDescription ?: @"摄像头打开失败"
                                           : @"摄像头已打开")];
        }];
    }
}
